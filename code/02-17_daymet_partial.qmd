...
#install.packages("sf") # Simple Features (Load shapes)
#install.packages("daymetr") # Data for climate variables
...
```{r}
states <- us_states() %>%
  filter(!(state_abbr %in% c("PR","AK","HI"))) #To filter more than one class and the ! is to flip the conditions
...
daymet_one %>%
  pivot_wider(names_from = measurement,
              values_from = value) # pivot wider is for bring rows to columns
...
Now let's unnest the weather column and do a quickly wrangling.  
```{r}
