...
# Introduction  
The practice of finding better representations of predictors (i.e., features) to improve predictive performance is referred to as “feature engineering.” - Derive new data from the data that we already have.
...
Now, what if we wanted to have month as part of the column name instead? Create a column for each month+weather variable.
...
fe_month_sum_wide <- fe_month_sum %>%
  ...
  mutate(across(.cols = c(3:75),
                ~round(., 1))) #. Represent each one of the columns, ~ is just convention to use across and a function inside.
...
ggplot(data = fe_month_sum,
  ...
  labs(x = "Mean Maximum Temperature C")
```
